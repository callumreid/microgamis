...
function AdviseTheChildGame({
  ...
  const [currentScenario, setCurrentScenario] = useState<ChildScenario | null>(
    null
  );
  ...
  useEffect(() => {
    if (!isInitialized) {
      ...
      if (sendVoiceMessage) {
        sendVoiceMessage(
          `A child comes to you with a problem and needs your wise advice. Listen carefully to what they have to say, then give them the best guidance you can. Here's what they say: ${randomScenario.childQuote}`
        );
      }
    ...
    if (onVoiceInput && !hasAnswered && currentScenario && isInitialized) {
      const handleVoiceInput = (transcript: string) => {
        ...
        if (input.length > 20) {
          ...
          const goodKeywordCount = currentScenario.goodAdviceKeywords.filter(
            (keyword) => input.includes(keyword)
          ).length;

          const badKeywordCount = currentScenario.badAdviceKeywords.filter(
            (keyword) => input.includes(keyword)
          ).length;
          ...
          if (score >= 60) {
            updateMessage(
              "Excellent advice! The child feels supported and knows what to do."
            );
            if (sendVoiceMessage) {
              sendVoiceMessage(
                "That was wonderful advice! You showed empathy, gave practical guidance, and helped the child feel supported. You'd make a great parent or mentor!"
              );
            }
            endGame(
              true,
              "Wise counselor! Your advice was caring and helpful.",
              score
            );
          } else if (score >= 30) {
            ...
            if (sendVoiceMessage) {
              sendVoiceMessage(
                "That was decent advice. You showed some understanding and gave some helpful guidance. The child feels a bit better."
              );
            }
            endGame(
              true,
              "Good guidance! The child feels somewhat helped.",
              score
            );
          } else {
            updateMessage(
              "The child looks confused and upset by your advice..."
            );
            if (sendVoiceMessage) {
              sendVoiceMessage(
                "Oh dear... your advice might not have been the most helpful. The child seems more confused or upset than before. Maybe try being more empathetic and constructive next time."
              );
            }
            endGame(
              false,
              "Poor advice. The child didn't feel helped.",
              Math.max(0, score)
            );
          }
        }
